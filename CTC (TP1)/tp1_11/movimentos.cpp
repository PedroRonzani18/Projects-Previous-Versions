...
void playerGeneralMovement()
{
    ...
    playerModelMovement();
    //generalHitboxMovement(&entityList[0]);
...
void shotsModelMovementAlone(int e)
{
...
void shotsModelMovementWithPlayer(int e)
{
...
void shotsGeneralMovement()
{
    for(int e=0; e<shotsList.size(); e++)
    {
        ...
        if(!shotsList[e].continuar) // se o tiro não tiver sido liberado, o player ainda controla sua posição
            shotsModelMovementWithPlayer(e);

        if(shotsList[e].continuar || on_off[6])
            shotsModelMovementAlone(e);
...
void shotsMovementWithPlayer(int e)
{
    ...
    //aux_angle = grausParaRadianos(shotsList[e].angulo);
