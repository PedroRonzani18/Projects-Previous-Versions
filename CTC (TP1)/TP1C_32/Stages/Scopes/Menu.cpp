...
Menu::Menu()
{
    ...
    Button startButton;
    startButton.setDisplayListModel(textures[8]);
    startButton.setCurrentPosition(1);
    startButton.setMidPoint(0,-20);
    this->addButton(startButton);

    Button helpButton;
    helpButton.setDisplayListModel(textures[6]);
    helpButton.setCurrentPosition(2);
    helpButton.setMidPoint(0,-60);
    this->addButton(helpButton);

    setAmmountOfButtons();

    Button emphasisButton;
    emphasisButton.setDisplayListModel(textures[4]);
    ...
    for(Button b : getButtons())
    {
        glPushMatrix();
            glTranslatef(b.getMidPoint().getX(),
                         b.getMidPoint().getY(),
                         0);
            if(typeid(b) == typeid(MovableEntity))
