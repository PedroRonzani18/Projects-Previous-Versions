...
void movimentaShots()
{
    if(dentroTela(&entityList[0]))
    {
        for(int e=0; e<shotsList.size(); e++){
            if(!shotsList[e].getContinuar()) // se o tiro não tiver sido liberado, o player ainda controla sua posição
            {
...
void atirar()
{
    for(int e=0; e<shotsList.size(); e++)
    {
        ...
        if(on_off[6] || shotsList[e].getContinuar() == 1)
        {
