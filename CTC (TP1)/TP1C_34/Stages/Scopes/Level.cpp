    ...
    for(int j=0; j<enemies.size(); j++)
    {
        for(int i=0; i<projectiles.size(); i++)
        {
            if(projectiles[i].getOwner() == 1)
            {
                if(colided(projectiles[i], enemies[j]))
                {
                    collisions.push_back(std::make_tuple(1, &projectiles[i], &enemies[j]));
                }
            }else if(projectiles[i].getOwner() == 2){
                if(colided(projectiles[i], player)){
                    collisions.push_back(std::make_tuple(2, &projectiles[i], &player));
                }
            }
        }

        if(colided(enemies[j], player)){
            collisions.push_back(std::make_tuple(3, &enemies[j], &player));
        }
    }

    for(int i=0; i<colectibles.size(); i++){
        if(colided(colectibles[i], player)){
            collisions.push_back(std::make_tuple(4, &colectibles[i], &player));
        }
    }
    */
...
void Level::collisionHandler()
{
    ...
    {
        switch (std::get<0>(collisions[i]))
        {
        case 1: //colisao dos tiros do player com o inimido
            Projectile *p = reinterpret_cast<Projectile*>(std::get<1>(collisions[i]));
            Enemy *e = reinterpret_cast<Enemy*>(std::get<2>(collisions[i]));

            break;
        
        case 2: //colisao dos tiros dos inimigos com o player
            Projectile *p = reinterpret_cast<Projectile*>(std::get<1>(collisions[i]));
            Player *e = reinterpret_cast<Player*>(std::get<2>(collisions[i]));
