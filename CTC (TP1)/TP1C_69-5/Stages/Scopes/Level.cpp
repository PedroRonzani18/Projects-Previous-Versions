...
void Level::callWaves()
{
    ...
    {
        std::vector<Enemy> auxiliar = waveCaller(waves[0]);
...
/*
    if(bossTime)
    {
        ...
        for(int i=0; i<boss.getEnemies().size(); i++)
        {
            this->getEnemy(i).getMidPoint().setX(this->getEnemy(i).getMidPoint().getX() + this->boss.getMidPoint().getX());
            this->getEnemy(i).getMidPoint().setY(this->getEnemy(i).getMidPoint().getY() + this->boss.getMidPoint().getY());
        }
...
*/
    ...
    for (int i = 0; i < enemies.size(); i++)
    {
        ...
        if(enemies[i].getTypeMove() == 10 || enemies[i].getTypeMove() == 22  || enemies[i].getTypeMove() == 23)
...
void Level::colider() // proibido.
{
    for (int j = 0; j < enemies.size(); j++)
    {
        for (int i = 0; i < projectiles.size(); i++)
        {
            ...
            {
                if (colided(projectiles[i], enemies[j]) && enemies[j].getType() != 10) // colisao dos tiros do player com o inimido
                {
        ...
        if (colided(enemies[j], player) && enemies[j].getType() != 10)
