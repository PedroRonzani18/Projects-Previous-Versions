...
int insideScreen(Entity& e)
{
    ...
    if(e.getMidPoint().getX() + e.getMax().getX() > right * aspectRatio){
        e.getMidPoint().setX(right * aspectRatio - e.getMax().getX());
        return 0;
    }


    if(e.getMidPoint().getX() + e.getMin().getX() < left * aspectRatio){
        e.getMidPoint().setX(left * aspectRatio - e.getMin().getX());
        return 0;
    }
