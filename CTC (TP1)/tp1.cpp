...
// Detecta colisões
int colisaoGeralEntidades(int e1, int e2)
{
    vertice iniDiag, fimDiag, iniLado, fimLado, deltaVet;
    double l=0, d1=0, d2=0;
    int overlap = 0;

    iniDiag.x = entityList[e1].centro.x;
    iniDiag.y = entityList[e1].centro.y;
    ...
    for(int i=0; i<4; i++)
    {
        ...
        fimDiag.x = entityList[e1].alteredHitbox[i].x;
        fimDiag.y = entityList[e1].alteredHitbox[i].y;
...
int colisaoGeralShots(int e1, int e2)
{
    vertice iniDiag, fimDiag, iniLado, fimLado, deltaVet;
    double l=0, d1=0, d2=0;
    int overlap = 0;

    iniDiag.x = shotsList[e1].centro.x;
    iniDiag.y = shotsList[e1].centro.y;
    ...
    for(int i=0; i<4; i++)
    {
        ...
        fimDiag.x = shotsList[e1].alteredHitbox[i].x;
        fimDiag.y = shotsList[e1].alteredHitbox[i].y;
        ...
        for(int j=0, s=1; j<4; j++, s=(j+1)%4) 
        {
            ...
            shotsList[e1].centro.x += deltaVet.x * -1;
            shotsList[e1].centro.y += deltaVet.y * -1;
        }
    }
...
int colisaoGeralllll(entidade* e1, entidade* e2)
{
    vertice iniDiag, fimDiag, iniLado, fimLado, deltaVet;
    double l=0, d1=0, d2=0;
    int overlap = 0;
...
void timer(int t)
{
    ...
    for(int i=1; i < entityList.size(); i++)
    {
        ...
        if(colisaoGeralllll(&entityList[0],&entityList[i]))
        {
            endGame = 1;
    ...
    for(int i=0; i < shotsList.size(); i++)
    {
        ...
        /*if(colisaoGeralShots(0,0)) // entre o tiro e o personagem NESSA ORDEM
        {
            endGame = 1;
            glutTimerFunc(2000, timer, 16); // Se ocorrer a colisão, o período da timer é momentaneamente definido para
                                            // 2 segundos, para a mensagem de "Game Over" aparecer na tela durante 2 segundos.
        ...
        if(colisaoGeralllll(&shotsList[0],&entityList[0])) // entre o tiro e o personagem NESSA ORDEM
        {
            endGame = 1;
            glutTimerFunc(2000, timer, 16); // Se ocorrer a colisão, o período da timer é momentaneamente definido para
                                            // 2 segundos, para a mensagem de "Game Over" aparecer na tela durante 2 segundos.
